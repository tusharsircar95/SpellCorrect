...
2. Rank this list based on edit distnace, soundex code similarity
3. Make a second pass and select word that is most commonly used
...
float bigramComparisonScore(string input,string word); // returns score of comparison based on bigram matchings
int getKeyboardDistance(char char1,char char2); // get distance on keyboard between the two characters
...
int main()
{
    ...
    while(1)
    {
        ...
        cin>>s;
        predictWord(s);
        //cout<<bigramComparisonScore(s,a)<<endl;
    }
    return 0;
}


int getKeyboardDistance(char char1,char char2)
{
    return abs(keyboardColumn[char1-96] - keyboardColumn[char2-96]);
}
float bigramComparisonScore(string input,string word)
{
    ...
    {
        string bigramInput = input.substr(i,2);
        for(int j=0,l=word.length()-1; j<l; j++)
        {
            string bigramWord = word.substr(j,2);
    ...
    else
    {
        ...
        for(vector<string>::iterator it = wordList.begin(); it!=wordList.end(); it++)
        {
            ...
            if(select)
            {
                ...
                cnode.score = sxScore - (getEditDistance(s,word)) + (1*bigramComparisonScore(s,word));
        ...
        cout<<"Candidate list selected after 1st filter is as follows: "<<candidateList.size()<<"\n";
        ...
        float maxScore = candidateList.front().score;
        // after first filter for the candidate list
